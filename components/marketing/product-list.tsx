...
export default async function ProductList({ locale }: { locale?: string }) {
  ...
  const t = await getTranslations({ locale: language, namespace: "MarketingHome.productList" });
                ...
                <div className="mt-4 flex items-end justify-between gap-4">
                  <div>
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-sm text-[#d0d6e0]">{t("priceUnit")}</span>
                    </div>
                    <p className="mt-3 text-sm text-[#8a8f98]">
                      {t("priceRange", { min: product.minPrice, max: product.maxPrice })}
                    </p>
                  </div>
                  <Button asChild size="sm" className="rounded-[6px] bg-[#3155f6] px-4 font-[590] text-white hover:bg-[#4568ff]">
                    <Link href="/rental">{t("rent")}</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>

...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (countdown > 0) {
      ...
      // Keep countdown mode after the timer ends so the user can send another code.
    }
    return () => {
        ...
        <section className="space-y-6">
          <div className="border border-border rounded-xl bg-card overflow-hidden">
            ...
            <div className="p-6 space-y-10">
              <div className="flex flex-col xl:flex-row items-start gap-10">
                <div className="space-y-4 w-full xl:w-auto text-left">
                  <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("profile.previewLabel")}</Label>
                  <div className="flex items-center justify-between xl:justify-start gap-6 p-5 rounded-2xl bg-muted/20 border border-border/50">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium">{t("profile.current")}</p>
                    </div>
                    {hasAvatarChange && (
                      <>
                        ...
                        <div className="text-center space-y-2">
                          ...
                          <p className="text-[10px] text-primary font-bold">{t("profile.preview")}</p>
                        </div>
                      </>
                    )}
                  </div>
                    ...
                    <div className="flex p-1 rounded-lg bg-muted/50 border border-border/50 w-fit">
                      {AVATAR_STYLES.map((style) => (
                        <Button key={style.id} variant={activeStyle === style.id ? "secondary" : "ghost"} size="sm" onClick={() => setActiveStyle(style.id)} className={cn("h-8 px-4 text-xs font-medium transition-all flex items-center gap-2", activeStyle === style.id && "bg-background shadow-sm")}>
                          ...
                          {t.has(`avatarStyles.${style.id}`) ? t(`avatarStyles.${style.id}`) : style.name}
                        </Button>
                      ))}
                    </div>
                  </div>
                    ...
                    <AnimatePresence>
                      {hasAvatarChange && (
                        ...
                        >
                          ...
                          >
                            <X className="w-4 h-4 mr-1" /> {t("profile.reset")}</Button>
                          <Button
                            size="sm"
              ...
              <div className="grid gap-6 md:grid-cols-2 pt-10 border-t border-border/50 text-left">
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("profile.email")}</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("profile.displayName")}</Label>
            ...
            <div className="px-6 py-3 bg-muted/30 border-t border-border flex justify-between items-center text-[10px] text-muted-foreground uppercase tracking-wider">
              ...
              {user?.createdAt && <span>{t("profile.registeredAt")}{user.createdAt}</span>}
            </div>
          </div>
        </section>
          ...
          <div className="border border-border rounded-xl bg-card overflow-hidden">
            <div className="p-6 border-b border-border bg-muted/30">
              <h3 className="text-lg font-semibold flex items-center gap-2">
                ...
                {t("security.title")}</h3>
              <p className="text-sm text-muted-foreground mt-1">{t("security.description", { email: user?.email ?? "" })}</p>
            </div>

            <div className="p-6 space-y-8">
              <div className="flex items-center justify-between py-2">
                <div className="space-y-1 text-left">
                  <p className="text-sm font-medium">{t("security.resetPassword")}</p>
                  <p className="text-xs text-muted-foreground">{t("security.sendCodeDescription")}</p>
                </div>
                <Button
                ...
                >
                  ...
                  {step === 'idle'
                    ? t("security.sendCode")
                    : (countdown > 0 ? t("security.resendCode", { seconds: countdown }) : t("security.resend"))
                  }
                </Button>
              </div>
                ...
                {step === 'countdown' && (
                  ...
                  >
                    <div className="grid gap-6">
                      <div className="space-y-2">
                        <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("security.emailCode")}</Label>
                        <Input required value={resetForm.code} onChange={(e) => setResetForm({...resetForm, code: e.target.value})} placeholder={t("security.codePlaceholder")} className="w-full" maxLength={6} autoComplete="one-time-code" />
                      </div>
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("security.newPassword")}</Label>
                          <Input type="password" required value={resetForm.newPassword} onChange={(e) => setResetForm({...resetForm, newPassword: e.target.value})} placeholder={t("security.newPasswordPlaceholder")} className="w-full" />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-xs text-muted-foreground uppercase tracking-wider">{t("security.confirmPassword")}</Label>
                          <Input type="password" required value={resetForm.confirmPassword} onChange={(e) => setResetForm({...resetForm, confirmPassword: e.target.value})} placeholder={t("security.confirmPasswordPlaceholder")} className="w-full" />
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center justify-end gap-4">
                      <Button type="button" variant="ghost" onClick={() => setStep('idle')} className="text-muted-foreground">{t("security.cancel")}</Button>
                      <Button type="submit" disabled={isLoading} className="min-w-[140px]">
                        {isLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Check className="w-4 h-4 mr-2" />}
                        {t("security.submit")}</Button>
                    </div>
                  </motion.form>
                )}
              </AnimatePresence>

...
export default function RechargePage() {
  ...
  const columns: DataTableColumn<RechargeOrderResponse>[] = [
    { key: "rechargeNo", title: t("columns.rechargeNo"), render: (row) => <span className="font-mono text-xs">{row.rechargeNo}</span> },
    { key: "applyAmount", title: t("columns.amount"), render: (row) => <MoneyText value={row.applyAmount} /> },
    { key: "channelName", title: t("columns.channel"), render: (row) => <span>{row.channelName} / {row.network}</span> },
    { key: "status", title: t("columns.status"), render: (row) => <StatusBadge status={row.status} /> },
    { key: "reviewRemark", title: t("columns.reviewRemark"), render: (row) => row.reviewRemark || "-" },
      ...
      render: (row) => (
        <div className="flex justify-end gap-2">
          ...
          {row.status === "SUBMITTED" ? (
            <ConfirmActionButton title={t("actions.cancelTitle")} description={t("actions.cancelDescription")} confirmText={t("actions.cancelConfirm")} onConfirm={() => cancelOrder(row.rechargeNo)}>
  ...
  const detailSections: DetailSectionDef<RechargeOrderResponse>[] = [
    {
      title: t("detail.amountSection"),
      fields: [
        { label: t("detail.amount"), render: (d) => <MoneyText value={d.applyAmount} /> },
        { label: t("detail.currency"), render: (d) => d.currency || "USDT" },
        {
          label: t("detail.actualAmount"),
          render: (d) => {
            ...
            return (
              <span className={cn(
                ...
                {hasAmount ? `${d.actualAmount.toFixed(2)} ${d.currency || "USDT"}` : t("detail.pendingConfirm")}
              </span>
            );
          },
        },
    ...
    {
      title: t("detail.orderSection"),
      fields: [
        { label: t("detail.orderNo"), render: (d) => <CopyableSecret value={d.rechargeNo} canReveal={false} /> },
        { label: t("detail.channel"), render: (d) => <span>{d.channelName} · {d.network}</span> },
        { label: t("detail.accountNo"), render: (d) => <CopyableSecret value={d.accountNo} canReveal={false} /> },
        { label: t("detail.status"), render: (d) => <StatusBadge status={d.status} /> },
        { label: t("detail.userRemark"), render: (d) => d.userRemark || "-" },
      ],
    },
    {
      title: t("detail.timeSection"),
      fields: [
        { label: t("detail.submittedAt"), render: (d) => <DateTimeText value={d.createdAt} /> },
        { label: t("detail.reviewedAt"), render: (d) => <DateTimeText value={d.reviewedAt} /> },
        { label: t("detail.creditedAt"), render: (d) => <DateTimeText value={d.creditedAt} /> },
      ],
    },
    {
      title: t("detail.proofSection"),
      fields: [
        { label: t("detail.externalTxNo"), render: (d) => <CopyableSecret value={d.externalTxNo} canReveal={false} /> },
        {
          label: t("detail.paymentProof"),
          render: (d) => {
            ...
            if (isImage) {
              return (
                <a href={d.paymentProofUrl} target="_blank" rel="noopener noreferrer" className="block">
                  ...
                  <img src={d.paymentProofUrl} alt={t("detail.paymentProof")} className="h-24 w-auto rounded-lg border border-gray-200 object-cover dark:border-white/10" />
                </a>
              );
            }
        ...
        { label: t("detail.walletTxNo"), render: (d) => d.walletTxNo ? <CopyableSecret value={d.walletTxNo} canReveal={false} /> : <span className="text-muted-foreground">-</span> },
      ],
    },
  ];
    ...
    <div className="space-y-6">
      ...
      {successState ? (
        <div className="flex flex-col items-center rounded-xl border border-border bg-card px-6 py-16 text-card-foreground shadow-sm">
          ...
          <h2 className="mt-5 text-xl font-semibold text-foreground">{t("success.title")}</h2>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          {/* Left side: amount and channel */}
          <div className="space-y-5 xl:col-span-2">
            {/* Area A: top-up amount input */}
            <div className="rounded-xl border border-border bg-card p-5 text-card-foreground shadow-sm">
              <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Wallet className="h-4 w-4 text-primary" />
                {t("form.amountTitle")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">{t("form.amountDescription")}</p>
            ...
            {/* Area B: channel selection */}
            <div className="rounded-xl border border-border bg-card p-5 text-card-foreground shadow-sm">
              <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Copy className="h-4 w-4 text-primary" />
                {t("form.channelTitle")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">{t("form.channelDescription")}</p>
              {channelsLoading ? (
                <div className="mt-4 flex items-center justify-center py-8">
                  <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
                  <span className="ml-2 text-sm text-muted-foreground">{t("form.loadingChannels")}</span>
                </div>
              ) : channels.length === 0 ? (
                <div className="mt-4 rounded-lg border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
                  {t("form.noChannels")}</div>
              ) : (
          ...
          <div className="xl:col-span-1">
            <div className="sticky top-6 space-y-5">
              <div className="rounded-xl border border-border bg-card p-5 text-card-foreground shadow-sm">
                <h2 className="text-sm font-semibold text-foreground">{t("form.summary")}</h2>
                <div className="mt-4 space-y-3">
                  ...
                  <div className="border-t border-border pt-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-foreground">{t("form.actualPay")}</span>
        ...
        >
          <div className="space-y-2">
            ...
            <Select value={filters.status ?? "ALL"} onValueChange={(value) => setFilters((current) => ({ ...current, status: value === "ALL" ? undefined : value }))}>
              <SelectTrigger className="h-9 min-w-[140px] bg-background text-foreground"><SelectValue placeholder={t("records.allStatus")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">{t("records.allStatus")}</SelectItem>
      ...
      {/* Detail drawer */}
      <DetailDrawer data={detail} open={Boolean(detail)} title={t("drawerTitle")} subtitle={(data) => data.rechargeNo} sections={detailSections} onClose={() => setDetail(null)}>
        {() => (

...
import {
  disableAdminRegion,
  enableAdminRegion,
  getAdminRegionTranslations,
  getAdminRegions,
  updateAdminRegionTranslation,
} from "@/api/admin";
import type { AdminCatalogQuery, RegionResponse } from "@/api/types";
...
const initialFilters: Filters = { regionCode: "", status: "" };
...
export default function AdminRegionsPage() {
  ...
  const [editingRow, setEditingRow] = useState<RegionResponse | null>(null);
  const [translationRow, setTranslationRow] = useState<RegionResponse | null>(null);

  const loader = useCallback(async (params: AdminCatalogQuery) => (await getAdminRegions(params)).data, []);
  ...
  const buildQuery = (nextFilters: Filters): AdminCatalogQuery => ({
    ...
    region_code: nextFilters.regionCode || undefined,
    status: nextFilters.status ? Number(nextFilters.status) : undefined,
  });

  const openEdit = (row: RegionResponse) => {
    setEditingRow(row);
    setFormOpen(true);
  };

  const toggle = async (row: RegionResponse, enabled: boolean) => {
    setActionError(null);
    try {
      if (enabled) {
        await enableAdminRegion(row.id);
      } else {
        await disableAdminRegion(row.id);
      }
      await reload();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <Dialog open={Boolean(translationRow)} onOpenChange={(open) => !open && setTranslationRow(null)}>
        <DialogContent className="flex max-h-[85vh] max-w-4xl flex-col items-stretch overflow-y-auto">
          <DialogTitle className="sr-only">{tTranslations("regionTitle")}</DialogTitle>
          {translationRow ? (
            <AdminTranslationEditor
              title={tTranslations("regionTitle")}
              resourceKey={translationRow.id}
              fields={[{ key: "regionName", label: tTranslations("regionName") }]}
              load={() => getAdminRegionTranslations(translationRow.id)}
              save={(locale, values) => updateAdminRegionTranslation(translationRow.id, { locale, regionName: values.regionName })}

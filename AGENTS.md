...
开发时优先遵循以上技术栈。除非用户明确同意且有充分技术理由，不要引入新的 UI 框架、组件库、样式系统、表单库、图表库或请求客户端。后端接口API：http://localhost:8080/swagger-ui/index.html
...
- **响应式设计：页面开发必须同时进行响应式适配。确保同一页面在手机、平板和电脑等不同屏幕尺寸下均有合理的布局表现，优先使用 Tailwind 的响应式工具类（如 `sm:`, `md:`, `lg:`, `xl:`）来实现。**
...
1. 针对改动逻辑的定向测试
2. 最近的相关测试套件
3. 受影响区域的类型检查但是不要运行npm run lint指令
4. 只有需要时才做更广范围的项目验证
